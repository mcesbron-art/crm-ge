...
type Props = {
  taskLabel: string;
  onClose: () => void;
  onConfirm: (data: { reason: string; waitingFor: string | null; comment: string | null; followUpDate: string | null }) => Promise<void>;
};
...
/**
 * Ouverte au glisser-déposer d'une carte vers "Attente d'éléments" — le
 * déplacement n'est confirmé qu'à la validation de cette modale. Même
 * recette visuelle que TaskTimeModal (panneau latéral droit, header sombre
 * Playfair Display).
 */
export default function TaskWaitModal({ taskLabel, onClose, onConfirm }: Props) {
  const [reason, setReason] = useState<string | null>(null);
  const [waitingFor, setWaitingFor] = useState("");
  const [showComment, setShowComment] = useState(false);
  const [comment, setComment] = useState("");
  const [showFollowUp, setShowFollowUp] = useState(false);
  const [followUpDate, setFollowUpDate] = useState(todayLocal());
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const canSave = !!reason && !saving;
  ...
  return (
    <div onClick={onClose} className="modal-overlay-in" style={{ position: "fixed", inset: 0, background: "rgba(16,15,11,.5)", zIndex: 90, display: "flex", justifyContent: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} className="modal-slide-in" style={{ width: 440, maxWidth: "100%", height: "100%", background: "#F5F5F2", display: "flex", flexDirection: "column", boxShadow: "-30px 0 70px -20px rgba(16,15,11,.5)" }}>
        <div style={{ background: "#0A0A0A", padding: "22px 24px 20px", position: "relative", overflow: "hidden", flex: "none" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, position: "relative" }}>
            <span style={{ fontSize: 13, color: "#9A9078", fontWeight: 600, letterSpacing: ".04em", textTransform: "uppercase" }}>Attente d&apos;éléments</span>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "20px 24px 26px", display: "flex", flexDirection: "column", gap: 18 }}>
          <div>
            <label style={{ fontSize: 13, letterSpacing: ".08em", textTransform: "uppercase", color: "#A6A498", fontWeight: 700, display: "block", marginBottom: 8 }}>Motif de l&apos;attente</label>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {TASK_WAIT_REASONS.map(r => {
                const on = reason === r.value;
                return (
                  <span key={r.value} onClick={() => setReason(r.value)} style={{ padding: "7px 13px", borderRadius: 99, fontSize: 14.5, fontWeight: 600, cursor: "pointer", color: on ? "#fff" : "#5C5A52", background: on ? "#0A0A0A" : "#F5F4EF", border: `1px solid ${on ? "#0A0A0A" : "#E5E4DD"}` }}>{r.label}</span>
                );
              })}
            </div>
          </div>
          ...
          <div>
            <label style={{ fontSize: 13, letterSpacing: ".08em", textTransform: "uppercase", color: "#A6A498", fontWeight: 700, display: "block", marginBottom: 6 }}>Personne ou entité attendue (optionnel)</label>
            <input value={waitingFor} onChange={e => setWaitingFor(e.target.value)} placeholder="Ex. Client, Studio photo…" style={{ width: "100%", boxSizing: "border-box", fontFamily: "inherit", fontSize: 15.5, color: "#1C1B16", background: "#fff", border: "1px solid #E2E1DA", borderRadius: 9, padding: "9px 12px", outline: "none" }} />
          </div>

          {showFollowUp ? (
            <div style={{ fontSize: 15, color: "#5C5A52", lineHeight: 1.5 }}>
              Date de relance : <DatePickerField value={followUpDate} onChange={setFollowUpDate} disableFuture={false} disablePast />
              <span onClick={() => setShowFollowUp(false)} style={{ marginLeft: 8, fontSize: 14, color: "#A6A498", cursor: "pointer" }}>Retirer</span>
            </div>
          ) : (
            <span onClick={() => setShowFollowUp(true)} style={{ fontSize: 15, fontWeight: 600, color: "#B0892B", cursor: "pointer", width: "fit-content" }}>+ Ajouter une date de relance</span>
          )}
